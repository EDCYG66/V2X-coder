# -*- coding: utf-8 -*-
"""
Agent with switchable GNN backend (GATv2 or GraphSAGE) + full metric export
...
- Memory-friendly logging (power_log sampling + capped length)
"""
...
class Agent(BaseModel):
    def __init__(self,
                 ...
                 plot_dpi: int = 224,
                 replay_size: int = 200000,          # 回放池容量（默认 20 万）
                 power_log_stride: int = 4,          # 功率日志采样步长（每 4 步记录一次）
                 power_log_max: int = 200000):       # 功率日志长度上限
        self.weight_dir = 'weight'
        os.makedirs(self.weight_dir, exist_ok=True)
        self.env = environment

        self.gnn_type = (gnn_type or "gat").lower()
        # GNN 实例（GAT 分支已固定使用 v2，不再需要 attn_version）
        self.G = build_gnn(
            environment,
        ...
        # 回放池：容量可配置（默认 20 万）
        self.memory = ReplayMemory(model_dir, memory_size=int(replay_size))
        ...
        self.test_history = []
        self.power_log = []  # 仍然用 list，但加了采样和长度上限
